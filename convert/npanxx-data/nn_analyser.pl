...
my %CARRLOOKUP = ( A => 'GCNS', B => 'SWAY', C => 'NETD', 'X' => 'Trial');
...
$r->load_rate_file('/home/grant/quickdials.com/rate-decks/trial-x.csv', 'Trial', 'X');
...
open SAMPLE, '<', '/home/grant/quickdials.com/rate-decks/sample-spread.txt' || die "failed to open sample data: $!";
...
while (my $l = <SAMPLE>) {
	...
	my $seconds = int(chomp(substr($l,8)));
...
#	if ((! defined($nn->{'BestCarriers'})) ||
#	    (length($nn->{'BestCarriers'}) == 0) ||
#	    ($nn->{'BestCarriers'} eq ' ')) {
#		# happens when trial has rate that others don't
#		die "Sample $l has no best carriers";
#	}
...
printf '%12s  ', "Avg c/min";
for my $c (sort keys %CARRLOOKUP) {
	my $v = '          ';
	if (defined($averates{$c})) {
		my $avestr = sprintf '%1.4f', 100 * $averates{$c} / $sample{'TotalSeconds'};
		$v = substr('                 ',0, 10 - length($avestr)) . $avestr;
	}
	print $v;
}
